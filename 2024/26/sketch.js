class Snake {
  constructor() {
    ...
    this.stepSize = 0.5; // Size of each step
    this.maxLength = 10000; // Maximum length of the path
  ...
  draw(colour) {
    stroke(colour[0], colour[1], colour[2], 100);
    strokeWeight(1);
    noFill();
    beginShape();
...
function setup() {
  ...
  backgroundColor = color(random(100,255), random(100,255), random(100,255), 10);
  for (let i = 0; i < 50; i++) { // Create 5 snakes
    snakes.push(new Snake());
    colours.push([random(200,255), random(200,255), random(255)]);
  }
}
